...
namespace net_ops::server
{
    ...
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        if (stmt_insert_user_) { sqlite3_finalize(stmt_insert_user_); stmt_insert_user_ = nullptr; }
        if (stmt_get_user_)    { sqlite3_finalize(stmt_get_user_);    stmt_get_user_ = nullptr; }
    ...
    bool DatabaseManager::ValidateUser(const std::string &username, const std::string &password)
    {
        ...
        if (storedHashBlob == HashPassword(password, saltBlob)) return true;
    ...
    int DatabaseManager::CreateGroup(const std::string &group_name, int owner_id)
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        sqlite3_stmt *stmt;
        if (sqlite3_prepare_v2(db_, "INSERT INTO groups (name, owner_id) VALUES (?, ?);", -1, &stmt, nullptr) != SQLITE_OK) return -1;
    ...
    bool DatabaseManager::AddMemberToGroup(int user_id, int group_id)
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        sqlite3_stmt *stmt;
        if (sqlite3_prepare_v2(db_, "INSERT OR IGNORE INTO group_members (group_id, user_id) VALUES (?, ?);", -1, &stmt, nullptr) != SQLITE_OK) return false;
    ...
    bool DatabaseManager::IsGroupOwner(int group_id, int user_id)
    {
        std::lock_guard<std::mutex> lock(db_mutex_);
        sqlite3_stmt *stmt;
        if (sqlite3_prepare_v2(db_, "SELECT 1 FROM groups WHERE id = ? AND owner_id = ?;", -1, &stmt, nullptr) != SQLITE_OK) return false;
        sqlite3_bind_int(stmt, 1, group_id);
        sqlite3_bind_int(stmt, 2, user_id);
